...
def copy_output_folder(project_root: Path) -> None:
    # Copy: training_models_risks/Po_Invoice_Data  ->  Prediction Pipeline/Po_Invoice_Data
    src = (project_root / "training_models_risks" / "Po_Invoice_Data")
    dst_parent = (project_root / "Prediction Pipeline")
    dst = dst_parent / "Po_Invoice_Data"
...
def main():
    ...
    for i, script in enumerate(SCRIPTS, 1):
        ...
        if cp.stdout: print(tail(cp.stdout))
        if cp.returncode != 0:
            any_failed = True
