...
FEAT_PO     = Path("Po_Invoice_Data/po_output_features_df_risk_02_10.pkl")         # engineered features
...
if FEAT_PO.exists():
    ...
    columns_to_drop = ['main_risk_scenario']
...
df["rft_by_engine_7"] = df["rft_by_engine_7"].astype(int)
## renaming rft_by_engine_po to model_flag
df.rename(columns={"rft_by_engine_7": "model_flag"}, inplace=True)
#df.drop(columns=["model_flag"],inplace=True)
# === Encode Remaining Categorical Columns ===
categorical_cols = df.select_dtypes(include='object').columns.difference(['sub_risks'])
label_encoders = {}
...
y_multi = mlb.fit_transform(df["sub_risks"])
label_names = mlb.classes_
print("Risk classes:", mlb.classes_)

# Drop old risk columns and Sub Risks list
X = df.drop(columns=["sub_risks"])  
y = y_multi  # Multi-hot encoded matrix
joblib.dump(X.columns.tolist(), "Po_Invoice_Data/trained_feature_columns_02_10.pkl")
...
results_df["predicted_risks"] = predicted_labels

# View sample
print(results_df[["predicted_risks"]].head())

# === STEP 10: Save Model and Preprocessing Artifacts ===
joblib.dump(model, "Po_Invoice_Data/po_rf_multilabel_model_sub_risks_02_10.pkl")
joblib.dump(mlb, "Po_Invoice_Data/po_mlb_02_10.pkl")
joblib.dump(label_encoders, "Po_Invoice_Data/po_label_encoders_02_10.pkl")
joblib.dump(imputer, "Po_Invoice_Data/po_imputer_02_10.pkl")
